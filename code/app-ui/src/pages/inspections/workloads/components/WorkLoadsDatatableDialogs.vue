    ...
    <v-dialog transition="dialog-top-transition" max-width="600" v-model="show.delete">
      <v-card>
        <v-card-title class="text-uppercase primary--text text-h6 py-2">
          {{ inspector.activo ? "desactivar" : "activar" }} perfil
        </v-card-title>
        <v-divider></v-divider>
...
export default {
  ...
  methods: {
    ...
    async viewInspector() {
      try {
        ...
        let [modules, usuarios, permissions, { perfil }] = await Promise.all([
          ...
          services.admin().getPermissionsFromInspector({ id }),
          services.admin().getEditInspectorInfo({ id }),
        ]);
        ...
        const { modulos, permisos, ...inspector } = perfil;
        ...
        if (modules.length > 0) this.inspector.modulos = modules;
        if (usuarios.length > 0) this.inspector.usuarios = usuarios;
        if (permissions.length > 0) this.inspector.permisos = permissions;
    ...
    async deleteInspector() {
      try {
        const { message } = await services.admin().deleteInspector(this.inspector.id);
