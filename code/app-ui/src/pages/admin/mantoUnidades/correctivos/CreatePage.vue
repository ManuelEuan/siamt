              ...
              <v-row>
                <v-col cols="6" md="3">
                  <v-autocomplete
                    ...
                    dense
                    outlined
                    v-model="correctivo.unidadId"
                    item-text="numeroEconomico"
                    item-value="id"
                    @change="infoUnidad"
                  ></v-autocomplete>
                </v-col>
                ...
                <v-col cols="6" md="3">
                  <v-text-field
                    type="number"
                    v-model="correctivo.costo_total"
                    label="Costo del servicio*"
                    :rules="[rules.required, rules.positiveIntNumber]"
                    hide-details="auto"
                    clearable
                    dense
                    outlined
                ...
                <v-col cols="4" style="margin-top: -2%">
                  <v-file-input clearable dense outlined counter show-size label="Archivo de factura" accept="application/pdf" v-model="file" />
                </v-col>
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      const { id, message, statusCode } = await (this.createMode
      ...
      if (statusCode === FAILURE_CODE) {
        this.showError({ message: "Error al guardar el correctivo." });
        return;
      }

      if (this.file) {
        const formData = new FormData();
        formData.append("id", id ?? this.correctivo.id);
        formData.append("file", this.file);

        await services.mantounidades().uploadFactura(formData);
        this.file = null;
      }
